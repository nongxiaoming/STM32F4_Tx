...
static unsigned handle_buttons(uint32_t button, unsigned flags, void*data);
#include "_gui.c"
...
void GUI_SetHidden(struct guiObject *obj, uint8_t state)
{
...
uint8_t _GUI_CheckTouch(struct touch *coords, uint8_t long_press, struct guiObject *headObj)
{
    struct guiObject *modalObj = GUI_IsModal();
...
unsigned handle_buttons(uint32_t button, unsigned flags, void *data)
{
    (void)data;
    //When modal, we capture all button presses
    uint8_t modalActive = GUI_IsModal() ? 1 : 0;
    ...
    if (objSELECTED) {
        void(*press)(struct guiObject *obj, uint32_t button, uint8_t press_type) = NULL;
