...
guiObject_t *GUI_CreateXYGraph(guiXYGraph_t *graph, uint16_t x, uint16_t y, uint16_t width, uint16_t height,
...
void GUI_DrawXYGraph(struct guiObject *obj)
{
    ...
    #define VAL_TO_X(xval) \
        (uint32_t)(box->x + (((s32)(xval)) - graph->min_x) * box->width / (1 + graph->max_x - graph->min_x))
    #define VAL_TO_Y(yval) \
        (uint32_t)(box->y + box->height - (((s32)(yval)) - graph->min_y) * box->height / (1 + graph->max_y - graph->min_y))
    ...
    uint16_t lastx = box->x;
    uint16_t lasty = box->y + box->height -1;
