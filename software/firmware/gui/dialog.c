...
guiObject_t *GUI_CreateDialog(guiDialog_t *dialog, uint16_t x, uint16_t y, uint16_t width, uint16_t height, const char *title,
...
void GUI_DrawDialog(struct guiObject *obj)
{
    ...
    LCD_GetStringDimensions((const uint8_t *)str, &dialog->txtbox.width, &dialog->txtbox.height);
...
void DialogClose(struct guiObject *obj, uint8_t state)
{
    ...
    void (*func)(uint8_t, void*) = dialog->CallBack;
    GUI_RemoveObj(obj);
    func(state, data);
}
