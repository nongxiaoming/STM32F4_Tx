...
static unsigned button_cb(uint32_t button, unsigned flags, void *data);
...
guiObject_t *GUI_CreateScrollbar(guiScrollbar_t *scrollbar, uint16_t x, uint16_t y, uint16_t height,
    uint8_t num_items, struct guiObject *parent,
    int (*press_cb)(struct guiObject *parent, uint8_t pos, s8 direction, void *data), void *data)
{
...
void GUI_DrawScrollbar(struct guiObject *obj)
{
    ...
    uint16_t bar_height = 10;  // Bug fix: After the logic view is introduce , a coordinate can be larger than 10000
    ...
    if (ARROW_UP == NULL) { // plate-text scroll bar for devo10
        ...
        uint16_t bar_y = scrollbar->cur_pos * (obj->box.height- bar_height)/(scrollbar->num_items -1)+ obj->box.y;
        ...
        uint8_t bar = scrollbar->cur_pos * (obj->box.height - 2 * ARROW_HEIGHT - bar_height) / (scrollbar->num_items -1);
...
void GUI_SetScrollbar(struct guiScrollbar *scrollbar, uint8_t pos)
{
...
uint8_t GUI_GetScrollbarNumItems(struct guiScrollbar *scrollbar)
{
