...
guiObject_t *GUI_CreateTextSelect(guiTextSelect_t *select, uint16_t x, uint16_t y, enum TextSelectType type,
...
guiObject_t *GUI_CreateTextSelectPlate(guiTextSelect_t *select, uint16_t x, uint16_t y, uint16_t width, uint16_t height, const struct LabelDesc *desc,
        void (*select_cb)(guiObject_t *obj, void *data),
        const char *(*value_cb)(guiObject_t *obj, int value, void *data),
        void *cb_data)
{
...
// only used for RTC config in Devo12
        ...
        LCD_GetStringDimensions((const uint8_t *)str, &w, &h);
        ...
        uint8_t arrow_width = ARROW_WIDTH - 1;
// only used for RTC config in Devo12
#if HAS_RTC
        uint8_t arrow_height = ARROW_HEIGHT - 1;
#endif
        if (select->enable  & 0x01) {
            uint16_t y = box->y + obj->box.height / 2;  // Bug fix: since the logic view is introduce, a coordinate could be greater than 10000
            uint16_t x1 = box->x + arrow_width -1;
            LCD_DrawLine(box->x, y, x1, y - 2, 0xffff);
            LCD_DrawLine(box->x, y, x1, y + 2, 0xffff); //"<"
            //LCD_DrawFastHLine(box->x, y, ARROW_WIDTH, 0xffff); //"-"
            x1 = box->x + box->width - arrow_width;
            uint16_t x2 = box->x + box->width -1;
...
void GUI_TextSelectEnablePress(guiTextSelect_t *select, uint8_t enable)
{
...
void GUI_TextSelectEnable(guiTextSelect_t *select, uint8_t enable)
{
    guiObject_t *obj = (guiObject_t *)select;
    enable = enable ? 1 : 0;
