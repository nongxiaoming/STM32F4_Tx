...
guiObject_t *GUI_CreateImageOffset(guiImage_t *image, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint16_t x_off, uint16_t y_off, const char *file,
...
void _GUI_ChangeImage(struct guiImage *image, const char *file, uint16_t x_off, uint16_t y_off, uint8_t replace)
{
    guiObject_t *obj = (guiObject_t *)image;
    //Use a CRC for comparison because the filename may change without the pointer changing
    uint32_t crc = Crc(file, strlen(file));
...
void GUI_ChangeImage(struct guiImage *image, const char *file, uint16_t x_off, uint16_t y_off)
{
    _GUI_ChangeImage(image, file, x_off, y_off, 0);
}

void GUI_ReplaceImage(struct guiImage *image, const char *file, uint16_t x_off, uint16_t y_off)
{
