...
static uint16_t  blink_fontcolor = 0xffff;
guiObject_t *GUI_CreateLabelBox(guiLabel_t *label, uint16_t x, uint16_t y, uint16_t width, uint16_t height, const struct LabelDesc *desc,
 ...
 */
void GUI_DrawLabelHelper(uint16_t obj_x, uint16_t obj_y, uint16_t obj_width, uint16_t obj_height, const char *str,
        const struct LabelDesc *desc, uint8_t is_selected)
{
    uint16_t txt_w, txt_h;
    uint16_t txt_x, txt_y;
    uint16_t w = obj_width;
    uint16_t h = obj_height;
    LCD_SetFont(desc->font);
    LCD_GetStringDimensions((const uint8_t *)str, &txt_w, &txt_h);
    if (desc->style == LABEL_BOX || desc->style == LABEL_BRACKET || desc->style == LABEL_SQUAREBOX) {
        ...
        txt_x = obj_x;
        txt_y = obj_y + 1;
        uint16_t old_w = obj_width;
        uint16_t old_h = obj_height;
        if (old_w < txt_w)
            old_w = txt_w;
        if (old_h < txt_h)
            old_h = txt_h;
