...
guiObject_t *GUI_CreateBarGraph(guiBarGraph_t *graph, uint16_t x, uint16_t y, uint16_t width, uint16_t height,
                      s16 min, s16 max, uint8_t direction,
...
void GUI_DrawBarGraph(struct guiObject *obj)
{
    ...
    if (val < graph->min)
        ...
        val = graph->max;
    uint32_t color = _bargraph_get_color(val, graph, disp);
