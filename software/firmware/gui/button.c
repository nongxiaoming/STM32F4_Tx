...
guiObject_t *GUI_CreateButton(guiButton_t *button, uint16_t x, uint16_t y, enum ButtonType type,
    const char *(*strCallback)(struct guiObject *, const void *), uint16_t fontColor,
...
guiObject_t *GUI_CreateIcon(guiButton_t *button, uint16_t x, uint16_t y, const struct ImageMap *image,
        void (*CallBack)(struct guiObject *obj, const void *data), const void *cb_data)
{
    struct guiHeader *obj = (guiObject_t *)button;
    struct guiBox    *box;
...
guiObject_t *GUI_CreateButtonPlateText(guiButton_t *button, uint16_t x, uint16_t y, uint16_t width, uint16_t height, const struct LabelDesc *desc,
    const char *(*strCallback)(struct guiObject *, const void *), uint16_t fontColor,
    void (*CallBack)(struct guiObject *obj, const void *data), const void *cb_data)
{
...
void GUI_DrawButton(struct guiObject *obj)
{
    ...
    if (button->image->file != NULL) {
        ...
        if (button->strCallback) {
            ...
            if (txt) {
                LCD_GetStringDimensions((uint8_t *) txt, &text_w, &text_h);
...
#define BUTTON_ROUND 3
        ...
        uint16_t text_w, text_h;
        uint16_t w = box->width;
        uint16_t h = box->height;
        LCD_GetStringDimensions((uint8_t *) txt, &text_w, &text_h);
        if (box->width == 0)
            w = text_w;
        if (box->height == 0)
        ...
        if (obj == objSELECTED) {
            ...
            uint16_t y1 = obj->box.y + 2;
            uint16_t y2 = obj->box.y + obj->box.height -3;
            uint16_t x1 = obj->box.x + obj->box.width - 1;
...
void GUI_ButtonEnable(struct guiObject *obj, uint8_t enable)
{
    struct guiButton *button = (struct guiButton *)obj;
...
uint8_t GUI_IsButtonEnabled(struct guiObject *obj)
{
    struct guiButton *button = (struct guiButton *)obj;
