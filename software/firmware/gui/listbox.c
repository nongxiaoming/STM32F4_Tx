...
#include "_listbox.c"
static int scroll_cb(struct guiObject *parent, uint8_t pos, s8 direction, void *data);
static unsigned press_cb(uint32_t button, unsigned flags, void *data);

guiObject_t *GUI_CreateListBox(guiListbox_t *listbox, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t item_count, s16 selected,
...
guiObject_t *GUI_CreateListBoxPlateText(guiListbox_t *listbox, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t item_count, s16 selected,
...
void GUI_ListBoxSelect(struct guiListbox *listbox, uint16_t selected)
{
    guiObject_t *obj = (guiObject_t *)listbox;
    listbox->selected = selected;
    if (listbox->select_cb)
        listbox->select_cb(obj, (uint16_t)selected, listbox->cb_data);
    OBJ_SET_DIRTY(obj, 1);
}
...
static int scroll_cb(struct guiObject *parent, uint8_t pos, s8 direction, void *data)
{
    ...
    if (direction > 0) {
        ...
        if(listbox->cur_pos != new_pos) {
            listbox->cur_pos = (uint16_t)new_pos;
...
uint8_t GUI_TouchListbox(struct guiObject *obj, struct touch *coords, uint8_t long_press)
{
    ...
    for (i = 0; i < listbox->entries_per_page; i++) {
        ...
        if (coords_in_box(&box, coords)) {
            uint8_t selected = i + listbox->cur_pos;
            if (selected != listbox->selected) {
                ...
                listbox->longpress_cb(obj, (uint16_t)selected, listbox->cb_data);
            }
            return 0;
        }
    }
...
{   // fix bug for issue #81: DEVO10: Model list should be browsable with UP/DOWN, so the listbox can change navigate key-sets now
    ...
    if ((flags & BUTTON_PRESS) || (flags & BUTTON_LONGPRESS)) {
        ...
        uint8_t move_down_button = BUT_RIGHT;
        uint8_t move_up_button = BUT_LEFT;
        if (listbox->style == LISTBOX_DEVO10) {
            if (listbox->key_style != LISTBOX_KEY_RIGHTLEFT) {
        ...
        if (CHAN_ButtonIsPressed(button, move_down_button)) {
            if (listbox->selected < listbox->item_count - 1) {
                ...
                if (listbox->select_cb)
                    listbox->select_cb(obj, (uint16_t)listbox->selected, listbox->cb_data);
                OBJ_SET_DIRTY(obj, 1);
            }
            ...
            if (listbox->longpress_cb)
                listbox->longpress_cb(obj, (uint16_t)listbox->selected, listbox->cb_data);
            else
                listbox->select_cb(obj, (uint16_t)listbox->selected, listbox->cb_data);
        }
        else {
