...
void kb_update_string(struct guiKeyboard *keyboard, uint8_t ch)
{
    uint8_t len = strlen(keyboard->text);
    if(ch == '\x08') {
        if (len > 0) {
...
static void kb_draw_key(struct guiBox *box, char c, uint8_t pressed)
{
    uint16_t w, h;
    uint16_t bg_color;
    uint16_t fg_color;
...
static uint8_t get_first_char(const char *str)
{
    const char *ptr = str;
    while(*ptr < 32)
        ptr++;
...
static uint8_t get_last_char(const char *str)
{
...
static char set_case(char c, uint8_t caps)
{
    if (! caps && c >= 'A' && c <= 'Z')
        return (c - 'A' + 'a');
    return c;
...
static char keyboard_cmd(enum DrawCmds cmd, struct guiKeyboard *keyboard, struct touch *coords)
{
    ...
    for(row = 0; row < 4; row++) {
        ...
        uint8_t first = get_first_char(ptr);
        uint8_t last  = get_last_char(ptr);
        uint8_t num_chars = 1 + last - first;
        uint16_t x_off = (LCD_WIDTH - KEY_W1 * num_chars) / 2;
        uint16_t y_off = Y_OFFSET + KEY_H * row;
        ...
        for(i = 0; i < strlen(ptr); i++) {
            ...
            if (cmd == KB_DRAW) {
                if ((c == '\x01' || c == '\x02') && (lastchar == '\x01' || lastchar == '\x02'))
                    keyboard->lastchar = c;
                uint8_t pressed = c == keyboard->lastchar;
