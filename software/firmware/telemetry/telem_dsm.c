...
int32_t _dsm_value(struct Telemetry *_t, int idx)
{
...
const char * _dsm_name(char *str, uint8_t telem)
{
    switch (telem) {
      case TELEM_DSM_FLOG_VOLT1: 
...
static const char * _dsm_short_name(char *str, uint8_t telem)
{
    switch(telem) {
...
int32_t _dsm_get_max_value(uint8_t telem)
{           
    switch(telem) {
        case TELEM_DSM_FLOG_FADESA:
        case TELEM_DSM_FLOG_FADESB:
